...
class cyclebase_model(object):
    def __init__(self, sess, args):
        ...
        self.D_loss_GX = md.least_square(self.sample_GX, tf.zeros_like(self.sample_GX))
        ...
        generator_loss_list = [self.G_loss_sum, self.G_loss_X2Y_sum, self.G_loss_Y2X_sum]
        if args.cycle_loss:
        ...
        for epoch in range(self.start_epoch, args.end_epoch):
            ...
            for _ in range(0, epoch_size):
                ...
                _, summary_str = self.sess.run(
                    ...
                    feed_dict = {self.real_X:X , self.real_Y:Y, \
                                 self.sample_GX:G_X, self.sample_FY:F_Y, self.lr:lr})
    ...
    def check_sample(self, args, idx):
        ...
        whole_X_image, whole_Y_image  \
        =  self.test_image_loader.LDCT_images[sltd_idx], \
            self.test_image_loader.NDCT_images[sltd_idx]

        G_X = self.sess.run(
            self.test_G_X,
            feed_dict={self.whole_X: whole_X_image.reshape(\
             [1] + self.whole_X.get_shape().as_list()[1:]), 
            self.whole_Y: whole_Y_image.reshape(\
             [1] + self.whole_Y.get_shape().as_list()[1:])})
    ...
    def test(self, args):
        ...
        for idx in range(len(self.test_image_loader.LDCT_images)):
            ...
            mk_G_X = self.sess.run(self.test_G_X,\
             feed_dict={self.whole_X: test_X.reshape([1]\
              + self.whole_X.get_shape().as_list()[1:])})
